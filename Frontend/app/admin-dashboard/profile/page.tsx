...
export default function AdminProfilePage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        if (isMockAuthEnabled()) {
          // Provide mock admin user for development
          const mockUser = {
            name: "Admin User",
            email: "admin@cyber.local",
            phone: "+91 98765 43210",
            department: "CERT-Army Division",
            avatar: null,
          }
          setUser(mockUser)
          setFormData({
    ...
    const fetchStatsAndActivity = async () => {
      try {
        if (isMockAuthEnabled()) {
          // Provide mock stats and activity for development
          setStats({
            total_incidents: 156,
            total_users: 42,
            resolution_rate: 87,
          ...
          setRecentActivity([
            { id: "1", action: "Reviewed incident #1234", timestamp: new Date().toISOString() },
            { id: "2", action: "Updated user permissions", timestamp: new Date(Date.now() - 3600000).toISOString() },
            { id: "3", action: "Generated security report", timestamp: new Date(Date.now() - 7200000).toISOString() },
            { id: "4", action: "Approved access request", timestamp: new Date(Date.now() - 10800000).toISOString() },
          ])
          return
        }
